...
function AdminPage({ isAdmin, onLogin }) {
  ...
  const fetchCategories = async () => {
    ...
    try {
      ...
      let nominees = [];
      try {
        const nomResp = await fetch('/api/admin/nominees', {
          headers: { 'X-Admin-Key': ADMIN_KEY }
        });
        if (nomResp.ok) {
  ...
  const handleAddDescription = async (e) => {
    ...
    try {
      // NOTE: Assuming the backend uses a PUT or PATCH endpoint for updating a category's properties
      const response = await fetch(`/api/admin/categories/${newDescription.category_id}`, {
  ...
  const handleEditNominee = async (nom, categoryId) => {
    ...
    try {
      const response = await fetch(`/api/admin/nominees/${nom.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json', 'X-Admin-Key': ADMIN_KEY },
        body: JSON.stringify({ name: newName }),
      });
